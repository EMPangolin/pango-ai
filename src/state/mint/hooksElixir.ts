...
export function useDerivedMintInfo(existingPosition?: Position): DerivedMintInfo {
  ...
  const [currencies, tokenA, tokenB, baseToken] = useMemo(() => {
    const wrapCurrency = (currency: Currency | null | undefined) =>
      currency ? wrappedCurrency(currency, chainId) : undefined;
  ...
  const mockPool = useMemo(() => {
    if (tokenA && tokenB && feeAmount && price && !invalidPrice) {
      ...
      return new ElixirPool(tokenA, tokenB, feeAmount, feeAmount, currentSqrt, JSBI.BigInt(0), currentTick, []);
    } else {
      return undefined;
    }
