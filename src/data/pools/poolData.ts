...
export function usePoolDatas(poolAddresses: string[]): {
  ...
  const {
    ...
    error: error24,
    data: data24,
  } = useQuery<PoolDataResponse>(POOLS_BULK(block24?.number, poolAddresses), { client: dataClient, fetchPolicy: 'cache-first' })
  const {
    loading: loading48,
    error: error48,
    data: data48,
  } = useQuery<PoolDataResponse>(POOLS_BULK(block48?.number, poolAddresses), { client: dataClient, fetchPolicy: 'cache-first' })
  const {
    loading: loadingWeek,
    error: errorWeek,
    data: dataWeek,
  } = useQuery<PoolDataResponse>(POOLS_BULK(blockWeek?.number, poolAddresses), { client: dataClient, fetchPolicy: 'cache-first' })
  ...
  const formatted = poolAddresses.reduce((accum: { [address: string]: PoolData }, address) => {
    ...
    const initialFee = current ? parseInt(current.initialFee) : 0
