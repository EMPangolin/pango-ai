...
import { CHAINS, ElixirTrade, JSBI, Percent, Router, SwapParameters, SwapRouter, Trade, TradeType } from '@pangolindex/sdk'
...
function useSwapCallArguments(
  ...
  const contract: Contract | null = useMemo(() => {
    ...
    return  getRouterContract(chainId, library, account)
...
export function useSwapCallback(
  ...
  return useMemo(() => {
    if (!trade || !library || !account || !chainId || !deadline) {
                ...
                return contract.callStatic[methodName](...args, options)
                  ...
                  .catch(callError => {
                    ...
                    switch (callError.reason) {
                      case 'CanaryRouter: INSUFFICIENT_OUTPUT_AMOUNT':
                      case 'CanaryRouter: EXCESSIVE_INPUT_AMOUNT':
                        errorMessage =
