...
export default function Swap() {
  ...
  const { priceImpactWithoutFee } = computeTradePriceBreakdown(trade, chainId ?? ChainId.AVALANCHE);
  ...
  const [inputSymbol, setInputSymbol] = useState<boolean>(false);
  const [outputSymbol, setOutputSymbol] = useState<boolean>(false);
  ...
  const handleInputSelect = useCallback(
    inputCurrency => {
      ...
      inputCurrency.symbol == 'MOVR' ? setInputSymbol(true) : setInputSymbol(false);
    },
    [onCurrencySelection],
  );
  ...
  const handleOutputSelect = useCallback(
    outputCurrency => {
      ...
      outputCurrency.symbol == 'MOVR' ? setOutputSymbol(true) : setOutputSymbol(false);
    },
    [onCurrencySelection],
  );
  return (
    <>
      ...
      <TopText hidden={inputSymbol || outputSymbol ? false : true}>
        Bring MOVR from Moonriver with
        <RelayLink href={'https://app.relaychain.com/'} target={'_blank'}>
           Relay
        </RelayLink>
      </TopText>
              ...
              <AutoRow justify={isExpertMode ? 'space-between' : 'center'} style={{ padding: '0 1rem' }}>
                <ArrowWrapper clickable>
                  <ArrowDown
                    ...
                    color={currencies[Field.INPUT] && currencies[Field.OUTPUT] ? '' : ''}
                  />
                </ArrowWrapper>
