...
import PoolCardV2SuperV2 from '../../components/earn/PoolCardV2SuperV2';
...
export default function SuperFarmsV2() {
  ...
  const stakingRewardsExist = Boolean(
    typeof chainId === 'number' && (STAKING_REWARDS_SP_V2_INFO[chainId]?.length ?? 0) > 0,
  );
  ...
  useEffect(() => {
    Promise.all(
      ...
      const poolCards = stakingInfoData.map(stakingInfo => (
        <PoolCardV2SuperV2 key={stakingInfo.pid} MinichefStakingInfo={stakingInfo} />
      ));
