...
const FACTORY_ADDRESS = '0xcfba329d49c24b70f3a8b9cc0853493d4645436b' //canary factory

export const ERC721_WALLET_DATA = address => {
  const queryString = `
    {walletData(id:"${address}"){tokenId}}
    `
  return gql(queryString)
}

export const ERC721_STAKING_DATA = address => {
  const queryString = `
    {stakingData(id:"${address}"){tokenId}}
    `
  return gql(queryString)
}

export const JOE_PAIR_24 = id => {
  const queryString = `
    {pairDayData(id:"${id}"){volumeUSD}}
    `
  return gql(queryString)
} //0xcdfd91eea657cc2701117fe9711c9a4f61feed23-19143

export const PNG_PAIR_24 = id => {
  const queryString = `
    {pairDayData(id:"${id}"){dailyVolumeUSD}}
    `
  return gql(queryString)
} //0xcdfd91eea657cc2701117fe9711c9a4f61feed23-19143

export const PAIR_24 = id => {
  const queryString = `
    {pairDayData(id:"${id}"){dailyVolumeUSD}}
    `
  return gql(queryString)
...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "canarydeveloper/canarydex") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const GET_BLOCK_BEFORE = gql`
  query blocks($timestampTo: Int!) {
    blocks(first: 1, orderBy: timestamp, orderDirection: desc, where: { timestamp_lt: $timestampTo }) {
      number
    }
  }
`
...
export const GET_BLOCK_AFTER = gql`
  query blocks($timestampFrom: Int!) {
    blocks(first: 1, orderBy: timestamp, orderDirection: desc, where: { timestamp_gt: $timestampFrom }) {
      number
    }
  }
`
...
export const GLOBAL_CHART = gql`
  query canaryDayDatas($startTime: Int!, $skip: Int!) {
    canaryDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
...
export const GLOBAL_DATA = block => {
  const queryString = ` query canaryFactories {
      canaryFactories(
