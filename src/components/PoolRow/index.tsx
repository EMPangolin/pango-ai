...
export const PoolRow: FC<PoolRowProps> = ({ pool, onOpenAddLiquidityModal }) => {
  ...
  const listItems = bonusTokens?.[0]?.map((address: string) => {
    ...
    return (
      <img key={address} className="token-logo" src={logoUrl} alt="token logo" />
    );
  });
  ...
  const listItems2 = bonusTokens?.[0]?.map((address: string, index: number) => {
    ...
    const rewardPerSecondUSD = rewardMultipliers && decimals && priceUSD && (Number(rewardMultipliers?.[0]?.[index]) / (10**decimals)) * priceUSD;
  ...
  const aprList = Number(bonusEndTime) > Date.now() / 1000 && Number(bonusEndTime) !== 0
    ...
    <div className="mb-4 border p-4 rounded-md flex flex-col gap-4 lg:flex-row lg:justify-between lg:items-center">
      ...
      <div className="grid grid-cols-3 gap-3 lg:grid-cols-5 lg:w-3/4">
        ...
        <div>
          ...
          {aprList.length > 0 ? (
            <>
              <h5 data-tip={`Base APR: ${apr.toFixed(2)}%<br/>Bonus APR: ${totalBonusAPR.toFixed(2)}%`} className="lg:text-center" data-html={true} >
